...
Failure to meet deadlines is a schedule risk that can cause the project to run overtime. It is caused when an individual fails to take responsibility for their work and isn't able to complete what they had agreed to do. An example is when a team member agrees to take on some work that they didn't have the time to finish. 
...
Major scope creep is a cost risk that can cause the project to run overtime. It is caused when extra tasks are added to the project scope during the project lifetime. An example is when another requirement is added to the project specification.
...
The impact of bad documentation is that the method is incorrectly used or that the code is confusing to developers who are editing it. These effects could lead to bugs in the program which will increase the work for developers and testers.
